...
# ================================================
# test-authz-policies-sc3.sh - Authorization Policy Workshop Tests for Scenario 3
# ================================================
#
# This script runs connectivity tests for the Authorization Policy workshop
# in Scenario 3 (3 clusters, 2 accounts). It applies policies and tests their effects interactively.
#
# Usage:
#   ./scripts/test/test-authz-policies-sc3.sh                      # Uses default config
#   ./scripts/test/test-authz-policies-sc3.sh -c myconfig.sh       # Uses custom config
#   ./scripts/test/test-authz-policies-sc3.sh -e 1                 # Run specific exercise (1-7)
#   ./scripts/test/test-authz-policies-sc3.sh -e all               # Run all exercises
#   ./scripts/test/test-authz-policies-sc3.sh --cleanup            # Remove all policies
#
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -h|--help)
                ...
                echo "  4. Allow Cluster 2 to Cluster 1 (same account)"
                echo "  5. Allow Cluster 3 to Cluster 1 (cross-account)"
                echo "  6. Allow internal communication within Cluster 1"
                echo "  7. Explicit deny external account (Cluster 3)"
                echo ""
                echo "Examples:"
                echo "  $0 -e 1              # Run baseline tests"
                echo "  $0 -e 5              # Test cross-account allow policy"
...
load_config() {
    ...
    # Validate scenario
    if [ "$SCENARIO" != "3" ]; then
        log_error "This script is for Scenario 3 only (three ECS clusters, two accounts)"
    ...
    ECS_CLUSTER3="ecs-${CLUSTER_NAME}-3"
    ECS_HOSTNAME_1="echo-service.${ECS_CLUSTER1}.ecs.local:8080"
    ECS_HOSTNAME_2="echo-service.${ECS_CLUSTER2}.ecs.local:8080"
    ECS_HOSTNAME_3="echo-service.${ECS_CLUSTER3}.ecs.external:8080"

    log_info "Configuration:"
    log_info "  CLUSTER_NAME=$CLUSTER_NAME"
    log_info "  ECS Cluster 1: $ECS_CLUSTER1 (local account)"
    log_info "  ECS Cluster 2: $ECS_CLUSTER2 (local account)"
    log_info "  ECS Cluster 3: $ECS_CLUSTER3 (external account)"
    echo ""
}
...
clear_all_policies() {
    ...
    kubectl delete authorizationpolicy --all -n "$ECS_CLUSTER3" 2>&1 | grep -v "No resources found" || true
    sleep 3
}
...
run_connectivity_matrix() {
    ...
    # Test EKS -> Cluster 3 (external account)
    log_test "EKS -> Cluster 3 echo-service (external account)"
    result=$(kubectl exec "$EKS_SHELL_POD" -- curl -s --max-time 5 "$ECS_HOSTNAME_3" 2>&1)
    if echo "$result" | grep -q "hostname"; then
        log_pass "Connected successfully"
    else
        log_fail "Connection failed: $(echo "$result" | head -c 100)"
    ...
    # Test Cluster 3 -> Cluster 1 (cross-account)
    log_test "Cluster 3 -> Cluster 1 echo-service (cross-account)"
    result=$(ORIGIN_CLUSTER="$ECS_CLUSTER3" ./scripts/test/call-from-ecs.sh "$ECS_HOSTNAME_1" 2>&1)
    if echo "$result" | grep -q "hostname"; then
        log_pass "Connected successfully"
    else
        log_fail "Connection failed: $(echo "$result" | head -c 100)"
...
EOF
    ...
    log_info "  - EKS -> Cluster 3:       OK"
    log_info "  - Cluster 2 -> Cluster 1: BLOCKED"
    log_info "  - Cluster 3 -> Cluster 1: BLOCKED (cross-account)"
...
exercise_4_allow_cluster2() {
    print_exercise_header "6.4" "Allow Cluster 2 to Cluster 1 (Same Account)"
...
EOF
    ...
    log_info "  - Cluster 3 -> Cluster 1: BLOCKED (cross-account still blocked)"
    log_info "  - Cluster 1 internal:     BLOCKED"
}

exercise_5_allow_cluster3() {
    print_exercise_header "6.5" "Allow Cluster 3 to Cluster 1 (Cross-Account)"

    log_info "Adding allow-cluster-3-to-echo policy (cross-account)..."

    # Ensure previous policies exist
    kubectl get authorizationpolicy deny-all -n "$ECS_CLUSTER1" &>/dev/null || exercise_4_allow_cluster2

    apply_policy "allow-cluster-3-to-echo" "$ECS_CLUSTER1" "$(cat <<EOF
apiVersion: security.istio.io/v1
kind: AuthorizationPolicy
metadata:
...
EOF
    ...
    run_connectivity_matrix "After adding allow-cluster-3-to-echo (cross-account)"
    ...
    log_info "  - Cluster 3 -> Cluster 1: OK (cross-account allowed!)"
    log_info "  - Cluster 1 internal:     BLOCKED"
}

exercise_6_allow_internal() {
    print_exercise_header "6.6" "Allow Internal Communication (Cluster 1)"

    log_info "Adding allow-internal policy..."

    # Ensure previous policies exist
    kubectl get authorizationpolicy deny-all -n "$ECS_CLUSTER1" &>/dev/null || exercise_5_allow_cluster3
...
EOF
    ...
    log_info "  - EKS -> Cluster 3:       OK"
    log_info "  - Cluster 2 -> Cluster 1: OK"
    log_info "  - Cluster 3 -> Cluster 1: OK (cross-account)"
    log_info "  - Cluster 1 internal:     OK (allowed by policy)"
}

exercise_7_deny_external() {
    print_exercise_header "6.7" "Explicit Deny External Account"

    log_info "Resetting to deny-all + allow local, then adding explicit deny for external account..."
    clear_all_policies
...
EOF
    ...
    apply_policy "deny-external-account" "$ECS_CLUSTER1" "$(cat <<EOF
apiVersion: security.istio.io/v1
kind: AuthorizationPolicy
metadata:
...
EOF
    ...
    run_connectivity_matrix "With explicit deny for external account (Cluster 3)"

    log_info "Expected:"
    log_info "  - EKS -> Cluster 1:       OK"
    log_info "  - EKS -> Cluster 2:       OK"
    log_info "  - EKS -> Cluster 3:       OK"
    log_info "  - Cluster 2 -> Cluster 1: OK (same account allowed)"
    log_info "  - Cluster 3 -> Cluster 1: BLOCKED (explicit DENY)"
    log_info "  - Cluster 1 internal:     BLOCKED"
    echo ""
    log_info "Note: DENY rules are evaluated before ALLOW rules."
    log_info "This demonstrates isolating cross-account traffic while allowing same-account access."
}

exercise_8_summary() {
    print_exercise_header "6.8" "Policy Summary & Cleanup"
    ...
    log_info "Current policies on Cluster 3 (external account):"
    kubectl get authorizationpolicies -n "$ECS_CLUSTER3" -o wide 2>/dev/null || echo "  (none)"
    echo ""
    ...
    echo "  6. Cross-account policies work the same as same-account policies"
    echo ""
...
run_all_exercises() {
    ...
    exercise_6_allow_internal
    echo ""
    read -p "Press Enter to continue to Exercise 6.7..."
...
print_header() {
    ...
    echo "  Scenario 3: Three ECS Clusters (Cross-Account)"
    echo "=============================================="
    echo ""
}
...
main() {
    ...
    if [ -z "$EXERCISE" ]; then
        ...
        echo "  4) Allow Cluster 2 to Cluster 1 (same account)"
        echo "  5) Allow Cluster 3 to Cluster 1 (cross-account)"
        echo "  6) Allow internal communication"
        echo "  7) Explicit deny external account"
        echo "  8) Summary & cleanup"
        echo ""
    ...
    case "$EXERCISE" in
        ...
        5) exercise_5_allow_cluster3 ;;
        6) exercise_6_allow_internal ;;
        7) exercise_7_deny_external ;;
        8) exercise_8_summary ;;
